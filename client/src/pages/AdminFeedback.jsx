...
const AdminFeedback = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const csrfToken = document.cookie
        ...
        ?.split('=').slice(1).join('=');
      const res = await fetch(`/api/feedback/${deleteModalId}/`, {
        method: 'DELETE',
        credentials: 'same-origin',
        headers: csrfToken ? { 'X-CSRFToken': decodeURIComponent(csrfToken) } : {},
      });
      if (res.ok) {
        setFeedbacks(prev => prev.filter(f => f.id !== deleteModalId));
  ...
  const filtered = useMemo(() =>
    feedbacks.filter(f => {
      const userName = getFeedbackUserName(f);
            ...
            >
              ...
              <div className="af-card-header">
                <div className="af-user-info">
                  <div className="af-avatar">{getFeedbackUserName(fb).charAt(0).toUpperCase()}</div>
                  <div>
                    <div className="af-username">{getFeedbackUserName(fb)}</div>
                ...
                <button
                  ...
                  onClick={() => setDeleteModalId(fb.id)}
                >
                  🗑
                </button>
              </div>
