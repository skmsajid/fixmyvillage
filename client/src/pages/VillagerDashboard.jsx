...
const IssueCard = ({ issue, label }) => {
  ...
  const imageUrl = getImageUrl(issue.photo);
    ...
    <div className="progress-section">
      ...
      {total === 0 ? (
        ...
        <div className="crs-wrap">
          <div className="crs-marquee" style={{ animationDuration: `${duration}s` }}>
            {[...issues, ...issues].map((issue, idx) => (
              <IssueCard key={`${issue.id}-${idx}`} issue={issue} label={label} />
            ))}
          </div>
        </div>
      ) : (
        <div className="crs-static">
          {issues.map(issue => (
            <IssueCard key={issue.id} issue={issue} label={label} />
          ))}
        </div>
      )}
    </div>
...
const issues = data.map(i => ({
  ...
  const submitIssue = async () => {
    ...
    return showToast("Submission failed", "error");
}
        ...
        {loadingData ? (
          ...
          <div className={`mi2-grid${filteredMyIssues.length > 4 ? ' mi2-grid--scroll' : ''}`}>
            {filteredMyIssues.map((issue, idx) => (
              <MyIssueCard key={issue.id} issue={issue} label={label} idx={idx} />
            ))}
          </div>
        )}
      </div>
